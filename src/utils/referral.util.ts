...
async function checkReferralCode(referral_code: string) {
...
async function checkActivityUsed(referred_user_id: string, activity: string) {
    const { data, error } = await supabase
        .from('referral')
        .select('*')
...
async function recordReferral(referrer_user_id: string, referred_user_id: string, referral_code: string, activity: string, kiss_coins: number) {
...
async function handleReferral(referral_code: string, code: number, referred_user_id: string) {
